...
function setup() {
    ...
    asteroids = new Group();
    bullets = new Group();
    background_image= loadImage('./assets/space.jpg')
    planet_image= loadImage('./assets/planetdestroyed2.jpg')
    //buzz = AudioNode('yoitrax-warrior');
    //buzz.play();
...
/*function myPlay(){
    //var audio = new Audio('yoitrax-warrior.mp3');
   // audio.play();
   const music = new Audio('yoitrax-warrior.mp3,hexdump');
   music.play();
   music.loop =true;
...
function draw() {
    ...
    text(`Score: ${score}`, 4 * width / 5, 40);
    if (life < 2) fill(255, 0, 0)
    text(`Lives: ${life}`, width / 5, 40);
...
function endScreen(str, color) {
    background(planet_image)
    textSize(width / 10);
    if (color === 0) fill(255, 255, 255);
    else fill(0,255,255);
